...
- **`net.@tcpServe(port, handler)` is the raw TCP server layer: the runtime accepts
  ...
  that block's own join, so `^` stays alive while the server runs.** Each accepted
  ...
  returning. A bind failure raises
  [QN511](docs/tooling/errors.md#qn511--bind-failed) and exits. See
...
- **`@tcpRequest` resolving a hostname no longer stalls every other fiber on the
  scheduler.** The DNS lookup now runs on the runtime's blocking-call pool, which
  wakes the reactor once it has an answer; the calling fiber parks on that wakeup
  exactly the way it parks on socket readiness, so a slow lookup no longer blocks a
  server's other connections, due `@sleep`s, or other in-flight requests. The pool's
  base is the process's CPU count (a minimum of 4), started on the first blocking
  call and kept for the rest of the process; under more concurrent calls than base
  threads it grows by one thread per call that finds every existing thread busy, up
  to 16 threads in total, and a thread beyond the base exits once it finds no more
  calls waiting. Neither the base nor the ceiling is configurable. A numeric
