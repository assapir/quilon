...
/// QN500/QN501/QN502/QN507/QN508: a fail-loud runtime check reports through the same coded
/// frame a compile error does — `stderr` carries `error[QNxxx]:` literally — so their
/// reference examples are checked the same way the front-end ones are, just run instead of
/// checked.
#[test]
fn runtime_examples_raise_their_own_code() {
    for code in [
        ...
        Code::ReplaceEmptyFrom,
        Code::StackOverflow,
        Code::WriteFdNotWhole,
        Code::WriteFailed,
        Code::ReplaceInvalidCount,
    ] {
