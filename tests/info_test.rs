...
fn every_member_answers() {
    ...
    assert_exit_linked(
        "<< core.info\n\
         ^ = () -> Num => (platform().name().size > 0 ? 1 : 0) + (os().name().size > 0 ? 2 : 0) \
           + (quilonVersion().size > 0 ? 4 : 0) + (endianness().name().size > 0 ? 8 : 0)",
        15,
    );
}
...
fn pointer_width_is_sixty_four_or_thirty_two() {
    assert_exit_linked(
        "<< core.info\n^ = () -> Num => pointerWidth() ? | Width64 => 64 | Width32 => 32",
        64,
    );
}
...
fn the_import_is_required() {
    ...
    assert_rejected("^ = () -> Num => pointerWidth().bits()");
}
