...
"#;
    ...
    let pipeline_count = tokens.iter().filter(|t| t.kind == TokenKind::Pipeline).count();
...
fn test_function_with_params() {
    ...
    assert_eq!(tokens.iter().filter(|t| t.kind == TokenKind::TypeAnnotation).count(), 2);
}
...
fn test_all_comparison_operators() {
    ...
    assert_eq!(tokens.iter().filter(|t| t.kind == TokenKind::And).count(), 3);
}
...
"#;
    ...
    let open_count = tokens.iter().filter(|t| t.kind == TokenKind::BlockOpen).count();
    let close_count = tokens.iter().filter(|t| t.kind == TokenKind::BlockClose).count();
...
fn test_array_syntax() {
    ...
    assert_eq!(tokens.iter().filter(|t| t.kind == TokenKind::Comma).count(), 4);
