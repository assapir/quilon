...
|------|--------|
| `quilon fmt` / linter | ⬜ |
| `quilon test` — in-language test framework (`describe`/`it`, pluggable reporters, tests beside the code they test, blocks erased from every other command) | 🔨 (cases assert with the fail-fast `assert*`, so a run stops at the first failure; a report-every-failure matcher API is next) |
