...
| `io.@readStdin() -> Text` | Read one line from stdin (without the trailing newline). A [leaf IO primitive](../concurrency/README.md): it launches the read and returns a **deferred** `Text` forced on first strict use. Yields `""` at end-of-input. |
| `io.@streamFile(path :: Text, chunkSize :: Num, onChunk :: (Text) -> Bool) -> Result` | Read `path` in `chunkSize`-byte reads, calling `onChunk` once per chunk. A [leaf IO primitive](../concurrency/README.md) that runs **strictly**, in program order on the calling fiber. `Ok(bytesRead)` / `NotOk(message)`. |
| `io.streamFile(path :: Text, onChunk :: (Text) -> Bool) -> Result` | `io.@streamFile` with a default chunk size the runtime chooses (64 KiB). |
...
The module's names — `@` primitives included — are reached through its binding, and its
