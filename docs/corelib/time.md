...
The [`time.@sleep`](../concurrency/README.md) leaf IO primitive (a pause) and the monotonic `time.now()` clock.

| Function | Effect |
|----------|--------|
| `time.@sleep(seconds :: Num) -> $` | Pause the current fiber for `seconds` (a fractional `Num`); execution then continues in program order. Effect-only: it carries no value to defer or force. A [leaf IO primitive](../concurrency/README.md) (the `@` marker). |
...
The leaf `time.@sleep` carries the marker; `^` and any helper it calls are unmarked. See the
