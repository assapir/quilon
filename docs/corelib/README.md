...
`replace`, and `replaceAll` are native. See [`Text`](../types/text.md).) The [concurrency model](../concurrency/README.md) that governs the `@` leaf primitives (`@readStdin`, `@sleep`) is language semantics — see that section.
