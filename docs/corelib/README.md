...
| [`core.test.report`](test/report.md) | `<< core.test.report` | The [test harness](test/report.md) `quilon test` runs and the report it prints: `describe` / `it` and `reportSuite` / `reportCase` / `reportSummary`. Pulls in `core.test`. Beside code, import it [test-only](test/report.md#test-only-imports) with `<<?`. |
