...
|--------|--------|-------------------|
| [`core.io`](io.md) | `<< core.io` | Output to file descriptors and stdin: `print` / `eprint` / `write`, the `stdout` / `stderr` descriptors, and the deferred `io.@readStdin` line read. |
...
| [`core.time`](time.md) | `<< core.time` | Time primitives: the `time.@sleep` pause and the monotonic `now()` clock. |
...
`replace`, and `replaceAll` are native. See [`Text`](../types/text.md).) The [concurrency model](../concurrency/README.md) that governs the `@` leaf primitives (`io.@readStdin`, `time.@sleep`) is language semantics — see that section.
