...
| String interpolation — the backtick render operator ([reference](../types/text.md#string-interpolation-and-the-render-operator-)) | ✅ |
...
| [Signal trap](../concurrency/signals.md) (`!>`, [`core.process`](../corelib/process.md)): a top-level item, one per program, in the file that defines `^`; its arms match `Signal` by bare variant name, each arm's body a fresh fiber under the fiber-sharing check; an unwritten arm keeps the OS default, and a repeat arrival during a running arm is delivered once, after it returns | ✅ |
