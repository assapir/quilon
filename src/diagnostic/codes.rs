...
codes! {
    // Input
    SourceNotReadable = 0 => "the source file cannot be read",
    NotAQuilonSource = 1 => "the file is not a `.qn` source",
    ...
    InterpolationHole = 9 => "interpolation hole is not a single expression",
    ImportPathInterpolated = 10 => "import path with interpolation",
    NotAnImportedModule = 11 => "qualified name on a module that is not imported",
    AmbiguousTypeDeclaration = 12 => "ambiguous `{ }` type declaration",
    OperatorMemberMutable = 13 => "operator member declared with `:=`",
    VariantNotCapitalized = 14 => "sum-type variant not capitalized",
    SumTypeHasFields = 15 => "sum type with fields or a mutating method",
    BodyNotABlock = 16 => "function body is not a `< >` block",
    ...
    UnknownModule = 19 => "module not found",
    NotExported = 20 => "member not exported by its module",
    ...
    NotAFunction = 29 => "call on a value that is not a function",
    ...
    UninferableLambdaParameter = 43 => "lambda parameter type cannot be inferred",
    RecursiveFunctionNeedsReturnType = 44 => "recursive function without a return type",
    SiteIsImmutable = 45 => "write to a `Site` field",
    MisplacedSiteParameter = 46 => "`Site` parameter that cannot be filled",
    ...
    ComparisonOverloadNotBool = 49 => "comparison operator not returning `Bool`",
    ...
    MatcherTypeUnsupported = 62 => "matcher applied to a type it cannot read",
    ...
    RangeEndpointNotWhole = 71 => "range endpoint is not a whole number",
