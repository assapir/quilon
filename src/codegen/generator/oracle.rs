...
impl<'ctx> CodeGenerator<'ctx> {
    ...
    ///     whose result type was taken from a never-constructed variant's generic arm, or
    ///     a `:: Result` parameter's own position no DIRECT caller ever informs) has no
    ///     concrete LLVM type; it falls back to the canonical numeric payload
    ///     representation `f64`, matching how generic/unknown payloads are materialized
    ///     elsewhere (`payload_slot_type`). This is sound whenever the position is either
    ///     never actually constructed (dead for that value) or genuinely always `Num` —
    ///     the checker pins any position a DIRECT caller demonstrates is something else
    ///     (see `pin_result_parameters`), and rejects a method's, an overload member's, or
    ///     a truly unreferenced declaration's parameter outright, since none of those has
    ///     any caller to pin from at all. It is NOT sound for a parameter only ever
    ///     reached indirectly — forwarded through another function's own bare `:: Result`
    ///     parameter rather than called with a concrete `Ok`/`NotOk` argument directly —
    ///     where a non-`Num` payload still renders as `f64` here; this is the same
    ///     pre-existing risk every other `Generic` default on this list has always carried,
