...
/// The `core.text` function implementing the Text method `name`, or `None` for a method
/// that is a native primitive (or no Text method at all). The composable methods are
/// written in Quilon (`corelib/text.qn`) over the primitives; a member call to one lowers
/// to a plain call of the named function with the receiver as the first argument. Shared
/// by codegen (the lowering), reachability (a mention of the method reaches the
/// implementation), and the module loader (a program mentioning one gets `core.text`
/// merged in).
pub fn qn_text_impl(name: &str) -> Option<&'static str> {
    Some(match name {
        "split" => "__qn_text_split",
        "trim" => "__qn_text_trim",
        "contains" => "__qn_text_contains",
        "replace" => "__qn_text_replace",
        "replaceAll" => "__qn_text_replace_all",
        "repeat" => "__qn_text_repeat",
        _ => return None,
    })
}
