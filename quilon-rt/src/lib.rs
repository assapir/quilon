...
//! GC, the shared `QlSlice` ABI type, bounds-check and range-extent failure). Each `#[no_mangle]`
...
pub use mem::{
    __alloc, __alloc_array, __gc_init, __index_fail, __range_count, __range_endpoint, GcThread,
    check_range_count, check_range_endpoint, register_thread,
};
...
intrinsic_registry! {
    ...
    __range_count: extern "C" fn(i64, i64, *const QlSite) -> i64,
