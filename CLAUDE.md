...
6. **Runtime intrinsics** — `quilon-rt` (`__write_bytes`, grapheme counting via `unicode-segmentation`, Boehm GC glue), packaged as `libquilon_rt` — which bundles the collector's object too, so the archive alone is a complete runtime. Not stubs.
...
- Arrays and `Text` are both `{ ptr, i64 }` structs in LLVM (`Text` = `{ data, byte_len }`; arrays = `{ data, size }`). `Text` is a built-in type, no import. Its model is `Text = []Grapheme` (indices/lengths count graphemes; a grapheme is a length-1 `Text` via `.at`/`.graphemes()`). Only the primitive methods (including `split`/`replaceAll`/`replace`, all linear in bytes) are native intrinsics; the remaining composable ones (`trim`/`contains`/`repeat`) are Quilon in `corelib/text.qn` — the loader (`src/modules.rs::link`) merges that module implicitly (binding no name; a written `<< core.text` is rejected — member syntax is the only surface) into any program that uses one, and codegen lowers the member call to the module's qualified function (`core.text.trim`).
- Sum types (`Ok`/`NotOk`) are tagged unions (i8 tag + payload). `Num`/`Bool`/`Text` payloads all work end-to-end (`Ok(text)`), as does `Text` (and nested arrays) inside records/arrays — a pattern-bound payload carries its concrete type. Different variants may carry different concrete payload types at the same position (`Ok(Text)` alongside `NotOk(Num)`); one variant's position carries one concrete type across every value of that variant — check `docs/types/sum-types.md` and `tests/sum_*.rs` before assuming.
- **A body is a block.** A function declaration's and a method's body is always a `< >` block, one expression or many (`double = (x :: Num) => < x * 2 >`). A **lambda** is the exception and may write a bare expression, so callbacks stay on one line (`xs.map(x => x * 2)`). Match arms and ternary branches are expression positions, not bodies — a block there is still a parse error (issue #232). Enforced in `Parser::parse_body` (`src/parser/ast_parser/items.rs`).
- **No keywords** — symbol-based: `^` entry point, `<<` import, `>>` export, `?`/`|`/`_` pattern matching, `? :` ternary, `~` comments. There is **no loop construct** — iterate with array methods (`.each`/`.map`/`.filter`/`.reduce`) and recursion (self-tail-calls are lowered to loops). Consult the symbol table in `docs/README.md`.
- **Modules are qualified** (see `docs/modules/README.md`): `<< core.http` binds `http`, and exports are reached through the binding — `http.send(...)`, `http.Request { }`, `| http.Get =>` — with the full path (`core.http.send`) as the ambiguity escape. Implemented as a link-time rename pass (`src/ast/qualify.rs`, driven by `src/modules.rs`): imported items — privates included, which travel but resolve for no importer — take fully-qualified names, so the checker and codegen see one flat namespace. Imports claim their short names (a binding named `http` after the import is an error); an `@` primitive is reached through its binding like any other export (`io.@readStdin`); module overload sets are **closed** (a user's bare `print`/`now` is an unrelated function).
- **Assertions are compiler-provided** and stay bare: `assert(value, matcher)` (fatal, exit 5) and `expect(value, matcher)` (recorded, only inside a `test.it` case) over the matchers `equals`/`contains`/`not`/`isOk`/`isNotOk` — no import needed. Neither the entry points nor the matchers are written in `.qn` — a matcher holds a value of the type under test, which without generics would need a matcher type per type. Checked in `src/typechecker/checker/assertions.rs`, lowered in `src/codegen/generator/assertions.rs`. The harness lives in `corelib/test.qn` (`core.test`): `test.describe`/`test.it` and the `core.test.reportSummary` the synthesized test entry point calls, over `test.failAt`, the run's recorded state, and the case lifecycle. Its report is fixed — the colors and the per-group/per-case lines are written out inside those functions rather than behind exported helpers. See `docs/corelib/test/`.
