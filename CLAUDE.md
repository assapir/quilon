...
is the runner; `docs/corelib/test/` is the reference.
...
- **Assertions are compiler-provided**, like `print`: `assert(value, matcher)` (fatal, exit 101) and `expect(value, matcher)` (recorded, only inside an `it` case) over the matchers `equals`/`contains`/`not`/`isOk`/`isNotOk`. Neither the entry points nor the matchers are written in `.qn` — a matcher holds a value of the type under test, which without generics would need a matcher type per type. Checked in `src/typechecker/checker/assertions.rs`, lowered in `src/codegen/generator/assertions.rs`. The harness is split so its output is replaceable: `corelib/test/report.qn` (`core.test.report`) holds `describe`/`it` and the three `report*` functions, while `corelib/test.qn` (`core.test`) holds only what those are built from — `failAt`, the run's recorded state, and the case lifecycle — leaving those five names free for a suite that brings its own. See `docs/corelib/test/`.
