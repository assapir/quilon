...
other subcommand erases them, so tests may live in the file they test — and with nothing left
referencing it, the `<< core.test` those blocks needed is tree-shaken out of the build by
`src/ast/reachability.rs`. `src/test_command.rs` is the runner; `docs/corelib/test/` is the
reference.
...
- Arrays and `Text` are both `{ ptr, i64 }` structs in LLVM (`Text` = `{ data, byte_len }`; arrays = `{ data, size }`). `Text` is a built-in type, no import. Its model is `Text = []Grapheme` (indices/lengths count graphemes; a grapheme is a length-1 `Text` via `.at`/`.graphemes()`). Only the primitive methods are native intrinsics; the composable ones (`split`/`trim`/`contains`/`replace`/`replaceAll`/`repeat`) are Quilon in `corelib/text.qn` — the loader (`src/modules.rs::link`) merges that module implicitly into any program that uses one, and codegen lowers the member call to the module's `__qn_text_*` function.
...
- **Assertions are compiler-provided**, like `print`: `assert(value, matcher)` (fatal, exit 101) and `expect(value, matcher)` (recorded, only inside an `it` case) over the matchers `equals`/`contains`/`not`/`isOk`/`isNotOk`. Neither the entry points nor the matchers are written in `.qn` — a matcher holds a value of the type under test, which without generics would need a matcher type per type. Checked in `src/typechecker/checker/assertions.rs`, lowered in `src/codegen/generator/assertions.rs`. The harness lives in `corelib/test.qn` (`core.test`): `describe`/`it` and the `reportSummary` the synthesized test entry point calls, over `failAt`, the run's recorded state, and the case lifecycle. Its report is fixed — the colors and the per-group/per-case lines are written out inside those functions rather than behind exported helpers, so the module takes no name from an importer that no caller outside it needs. See `docs/corelib/test/`.
- I/O lives in the `core.io` module (`<< core.io`): `print`/`eprint`/`write`/`stdout`/`stderr`. There is no `println`. `print`/`eprint`/`write` take **anything renderable**: the compiler resolves the `` ` `` render member on the argument's type, calls it, and writes the resulting `Text` (the same path string interpolation takes). A type becomes printable by defining that member — nothing extends `print`; the compiler claims these three names at their own arity (1, 1, 2) and rejects a definition there. `core.time`'s `now` is still a built-in **overload set** member, and a user definition of it adds a member rather than shadowing it.
